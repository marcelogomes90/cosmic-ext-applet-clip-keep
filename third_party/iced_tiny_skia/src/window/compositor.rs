...
pub fn present(
    ...
    if damage.is_empty() {
        ...
        let Some(mut pixels) = tiny_skia::PixmapMut::from_bytes(
            bytemuck::cast_slice_mut(&mut buffer),
            physical_size.width,
            physical_size.height,
        )
        else {
            // A Wayland resize can briefly leave softbuffer at the previous
            // dimensions. Report the surface as lost so iced recreates it at
            // the current viewport size instead of crashing the application.
