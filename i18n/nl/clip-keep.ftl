search-placeholder = Zoeken
empty-history = Nog niets gekopieerd
no-results = Geen resultaten
capture-unavailable = Klembordopname is niet beschikbaar
settings = Instellingen
details = Details
back = Terug
section-pinned = Vastgezet
section-recent = Geschiedenis

clear-title = Geschiedenis wissen?
clear-detail = Vastgezette items blijven behouden.

item-image = Afbeelding { $width } × { $height }

details-information = Informatie
details-source = Gekopieerd uit
details-size = Afmetingen
details-copied = Eerst gekopieerd
details-used = Laatst gebruikt
details-copies = Aantal keer gekopieerd
details-bytes = Grootte
details-formats = Indelingen
format-plain = Platte tekst
format-files = Bestanden
details-moment-format = %d-%m-%Y %H:%M

section-privacy = Privacy
section-history = Geschiedenis
section-behaviour = Gedrag
section-links = Links

action-pin = Vastzetten
action-unpin = Losmaken
action-delete = Verwijderen
action-details = Details
action-cancel = Annuleren
action-clear = Wissen

footer-back = terug
footer-use = gebruiken

setting-max-entries = Grootte van geschiedenis
setting-max-age = Verwijderen na
setting-max-age-never = Nooit
setting-capture-images = Afbeeldingen opslaan
setting-image-size = Afbeeldingsgrootte
setting-image-size-small = Klein
setting-image-size-medium = Middel
setting-image-size-large = Groot
setting-paste-on-use = Automatisch plakken
setting-private-mode = Privémodus
setting-respect-password-hint = Wachtwoorden overslaan

setting-max-age-days = { $days ->
    [one] 1 dag
   *[other] { $days } dagen
}

link-issues = Problemen
link-developer = Ontwikkelaar
link-repository = Repository
